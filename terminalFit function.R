...
# This function fits concentration-time data to an exponential equation of the form 
# f(t) = A*exp(-k * t)
# where A is Cmax, k is the terminal elimination rate constant, and t is time.
# Input is:
# DF - the data.frame with the concentration-time data
# startValues - list of starting values for A and k
# concentration - which column in DF contains concentration data
# time - which column in DF contains time data
# tmax - the putative tmax, which is used for estimating the 
# terminal elimination rate. Time points BEFORE tmax will be omitted
# from the fit. 
# I wrote this script initially for bootstrapping, where it can be useful
# to see what data were used as input. For that reason, I'm including the
# option of returning the data that were used.
